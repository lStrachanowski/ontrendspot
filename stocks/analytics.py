...
def update_database():
    data_files_paths = stocks_files_paths(
        r"C:\\Users\\lukaa\Desktop\\d_pl_txt\\data\\daily\\pl\\wse stocks\\")
...
def get_stock_mean_volume_value(period, min_value):
    time_difference = datetime.today() - timedelta(days=period)
    data_from_db = DataSource.objects.filter(
        day__gte=time_difference).order_by('stock_symbol')
    df = pd.DataFrame.from_records(data_from_db.values_list())
    df = df.rename(columns={1: 'stock_symbol', 2: 'day', 3: 'volume',
                   4: 'stock_open', 5: 'stock_high', 6: 'stock_low', 7: 'stock_close'})
    df = df.drop(columns=[0])
